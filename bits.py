...
def read_query_data():
    try:
        with open('query.txt', 'r') as file:
            data = file.readline().strip()  # Read the first line and strip whitespace
            return data
...
def main():
    # Fetch tasks first to get available tasks
    tasks = fetch_tasks()
    if tasks:
        for item in tasks:
            ...
            if item['status'] == 'None':  # Check if the task is not claimed
                ...
                if start_task(task_name):  # Start the task
                    # After starting, claim the task
